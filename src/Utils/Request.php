...
class Request
{
	...
	public function __construct($agreementToken = '', $apiSecret = '', $stripNull = false, $baseUri = null, $contentType = 'application/json')
	{
		...
		$data = [
            'base_uri'        => $baseUri ?? rescue(function () { return app( 'economic.urls' )->get( 'request_endpoint' );}, function () { return config('economic.request_endpoint'); }, false),
            'headers'         => [
	...
	public function doRequest($method, $path, $options = [])
	{
		...
		if (filter_var(config('economic.retry_server_exceptions.enabled'), FILTER_VALIDATE_BOOLEAN)) {
			...
			}, config('economic.retry_server_exceptions.timeout_ms', 10000), function (\Throwable $throwable) { 
				return (int) $throwable->getCode() === 429
					|| (
